...
ws.Range("A2:O" & Cells(Rows.Count, 1).End(xlUp).Offset(2, 0).Row).Select
'For every row in the current selection...
For Counter = 1 To Selection.Rows.Count
    'If the row is an odd number (within the selection)...
...
ws.Range("A3:O" & Cells(Rows.Count, 1).End(xlUp).Offset(2, 0).Row).Select
For Counter = 1 To Selection.Rows.Count
    If Counter Mod 3 = 1 Then
        With Selection
            .Rows(Counter).Borders(xlDiagonalDown).LineStyle = xlNone
